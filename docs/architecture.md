...
DB 接続情報は `.env` などから環境変数として受け取り、コード側では抽象名のみを参照する運用を想定する。
...
- `adsb_viewer/settings.py` 側では、
  - `NAME="adsb_test"` / `USER="lab_ro"` / `HOST="127.0.0.1"` / `PORT="5432"` を固定値とし、
  - `PASSWORD` のみ `os.environ.get("PGPASSWORD", "")` から取得する。
